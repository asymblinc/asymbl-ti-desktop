...
- Connection status: real SF-token check (`auth-store.js`) + real mic-permission check (Electron `systemPreferences`). Calendar is never "connected" (no Google/Outlook integration exists — TODOS #13/§4.2's degraded state is the correct, honest rendering for now, not a bug).
